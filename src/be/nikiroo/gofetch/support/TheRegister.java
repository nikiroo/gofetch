	...
	BasicSnippetExtractor getSnippetExtractor() {
		return new BasicSnippetExtractor() {
			...
			protected String getArticleContent(Document doc, Element article) {
				// We have some "details" but no content, so we switch them:
				Element detailsElement = article.getElementsByClass(
						"standfirst").first();
				if (detailsElement != null) {
					BasicFullArticleExtractor hack = getFullArticleExtractor();
					return hack.getArticleText(detailsElement);
				}
	...
	BasicCommentExtractor getCommentExtractor() {
		return new BasicCommentExtractor() {
			...
			protected List<Element> getFullArticleCommentPosts(Document doc,
				...
				try {
					URL url = new URL(
							"https://forums.theregister.co.uk/forum/1"
									+ intUrl.getPath());
							...
							for (Element post : posts
								...
								if (inReplyTo != null) {
									...
									if (parentId != null
										...
										commentReplies.put(getCommentId(post),
												parentId);
									}
								}
							}
