	...
	BasicSnippetExtractor getSnippetExtractor() {
		return new BasicSnippetExtractor() {
			...
			protected String getArticleContent(Document doc, Element article) {
				Element listing = article.getElementsByClass("BlurbListing")
						.first();
				if (listing != null && listing.children().size() >= 2) {
					...
					for (int i = 1; i < listing.children().size() - 2; i++) {
						...
						content = content.trim() + " " + hack.getArticleText(e);
					}
	...
	BasicCommentExtractor getCommentExtractor() {
		return new BasicCommentExtractor() {
			...
			protected String getCommentAuthor(Element post) {
				Element detailsE = post.getElementsByClass("CommentPoster")
						.first();
				if (detailsE != null) {
					...
					if (pos >= 0) {
						details = details.substring(pos + " by ".length())
								.trim();
